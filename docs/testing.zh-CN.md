...
两台 Ubuntu/Niri 日常桌面均通过共用安装器升级，配置哈希和身份文件元数据保持不变。
通过真实托盘回调重复开始、停止共享；关闭窗口保留托盘和共享，从托盘退出则停止后端，
NiriBridge 虚拟输入设备数量归零。重新打开界面时共享保持停止，再明确开始共享后，
两条已配置连接均恢复就绪。

随后用户确认：单指移动、三指工作区切换、四指总览、两条实体跨屏入口往返，以及
停止共享后的本机触控板操作均正常。这是初始硬件上的实机验收，与虚拟机和模拟窗口
证据分别记录；长期休眠、更多设备型号、扩展按钮和组合手势仍需更广泛验证。

## KDE Plasma 0.3 测试

`cargo test --locked --test kde_capture -- --ignored --nocapture` 使用独立虚拟帧缓冲、临时目录和私有会话总线，不连接日常桌面，不打开物理输入设备。测试夹具直接连接其私有 KWin EIS；正式程序始终通过 Portal 授权。已在 KWin 6.6.6 验证桌面检测、屏幕信息、绝对与相对鼠标注入、边缘捕获、焦点与快捷键抑制、按钮、超时释放和撤销授权。已有 Niri 捕获与加密协调器回归也已通过。

实机 KDE 双屏逻辑布局与 logind 会话识别已经只读验证。用户已确认两台实机的 KDE/Niri 双向键鼠与正常返回；Breeze 标题栏按钮比例已修正，并已用实际 GTK 主题与窗口按钮布局在隔离桌面核对。原生 KDE 触摸板手势、锁屏与重连恢复、紧急返回和长时间休眠仍需单独验收，不能从基本键鼠通过推断这些项目已通过。

## 配对文件选择窗口回归

原生文件选择窗口测试只保留弱观察引用，以捕获对话框被提前回收的问题。测试覆盖公钥导出到临时文件、导入进入指纹核对、取消、重复点击和父窗口销毁，不修改真实配对，不启动共享。
