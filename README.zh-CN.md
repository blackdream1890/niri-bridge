...
从 [GitHub Releases](https://github.com/blackdream1890/niri-bridge/releases) 下载 Ubuntu 26.04 amd64 **`.deb` 软件包**，使用 `SHA256SUMS` 校验后，交给系统的软件包安装器打开，也可执行 `sudo apt install ./niri-bridge-0.3.0-beta.3-ubuntu26.04-amd64.deb`。系统会安装所需依赖，完成后从应用启动器打开 **NiriBridge**。
...
打开 NiriBridge 后，点击“开始共享”才启动共享；“停止共享”保留界面，托盘中的“退出 NiriBridge”会先停止共享再退出应用。两台电脑应一起升级。安装程序会安全停止旧版、自动备份旧程序和设置，并保留身份、配对、设备权限和自定义服务。
...
- **屏幕连接**：添加多对出入口，分别选择显示器、边缘和范围；拖动对方屏幕调整选中的连接。在一台保存即可通过现有加密连接同步全部连接。
- **设备与配对**：导出本机公开配对文件，检查对端文件并核对完整指纹后确认信任。
- **偏好设置**：选择实体输入设备、启用原生触摸板手势、管理自动启动和界面语言。
- **系统托盘**：打开界面、开始或停止共享、退出 NiriBridge。关闭窗口会隐藏到托盘，从托盘退出会停止共享。

界面管理标准的 `niri-bridge.service` 用户服务，并识别手动运行的独立进程，避免操作错误的实例。登录启动通过 `niri-bridge-ui.service` 只打开界面，共享仍由用户手动开始。
...
可点击“停止共享”或从托盘退出 NiriBridge。命令行停止方式为：
