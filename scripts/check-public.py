...
def main():
    ...
    parser.add_argument('--history', action='store_true', help='Require private commit metadata and scan every historical blob.')
    ...
    if args.archive:
        ...
        if args.history:
            ...
            for commit in commits:
                addresses = subprocess.check_output(['git', '-C', str(root), 'show', '-s', '--format=%ae%n%ce', commit], text=True).splitlines()
                if not all(address.endswith('@users.noreply.github.com') for address in addresses):
                    issues.append({'commit': commit[:12], 'reason': 'non-private commit email'})
    ...
    print(json.dumps({'files_checked': checked, 'vendored_packages_verified': len(vendor_files), 'issues': issues}, indent=2))
